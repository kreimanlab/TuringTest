...
var TuringExperiment = function() {
    ...
    $.ajax({
        url: "/static/dataset_new/correct_answers.json",
        dataType: "json",
        async: false,
    ...
    var wordon;
    const NumConvPerWorker = 32 //32; // 32 total trials
    ...
    // Define the fixed position conversations for new dataset
    var fixedConversations = {
        7: _.shuffle([267, 268, 269, 270, 271, 272, 273, 274, 275, 276, 277, 278, 279, 280, 281])[0], // AIAI
        15: _.shuffle([34, 55, 59, 92, 102, 112, 119, 151, 152, 161, 169, 170, 172, 282, 283, 284])[0], // Human-AI
        25: _.shuffle([163, 166, 173])[0] // Human-Human
    };
    
    // Shuffle the catch conversations and select 3
    catchConversations = _.shuffle(catchConversations);
    var selectedCatchConvs = catchConversations.slice(0, 3);

	//Record catch trials and fixed trials
	psiTurk.recordUnstructuredData("catch_trials", selectedCatchConvs);
    psiTurk.recordUnstructuredData("fixed_trials", fixedConversations);

    
    // Define fixed positions for catch conversations (e.g., positions 4, 10, 20)
    var catchPositions = [4, 10, 20]; // These are 0-indexed positions
    
    // Define fixed positions for the new fixed conversations
    var fixedPositions = [7, 15, 25]; // These are 0-indexed positions
    ...
    // Extract conversation numbers from correct_answers.json
    var conversationNumbers = new Set();
    for (var key in correctAnswerList) {
        if (key.includes("_len3")) { // Use len3 as representative
            var convNum = parseInt(key.match(/conv(\d+)_len3/)[1]);
            conversationNumbers.add(convNum);
        }
    }
    
    // Convert to array for easier handling
    var availableNumbers = Array.from(conversationNumbers);
    
    // Categorize conversations by type
    for (var num of availableNumbers) {
    ...
    // Set counts for balance (adjusted to account for 3 fixed trials)
    var hhCount = 7; // Reduced from 8 (one HH is fixed at position 25)
    var aiaiCount = 7; // Reduced from 8 (one AIAI is fixed at position 7)
    var aihCount = 6; // Same as before
    var haiCount = 5; // Reduced from 6 (one HAI is fixed at position 15)
    ...
    // Add fixed conversations at their specified positions
    for (var i = 0; i < fixedPositions.length; i++) {
        var position = fixedPositions[i];
        var convNum = fixedConversations[position];
        
        // Determine the type of this fixed conversation
        var key = "conv" + convNum + "_len3";
        var type = correctAnswerList[key];
        ...
        if (position <= selectedConvs.length) {
            selectedConvs.splice(position, 0, fixedConv);
        } else {
            selectedConvs.push(fixedConv);
        }
    }
    ...
    // For fixed conversations, set specific lengths
    for (var i = 0; i < selectedConvs.length; i++) {
        if (selectedConvs[i].isFixed) {
            lengthlist[i] = 24; // Fixed conversations use length 24
        }
    }
    ...
    for (var i = 0; i < selectedConvs.length; i++) {
        ...
        if (conv.isCatch) {
            var conversationname = "/static/dataset_new/catch/conv" + conv.num + ".html";
            conversationlist.push(conversationname);
            
            // Assuming you have option files for catch conversations too
            var option1name = "/static/dataset_new/catch/conv" + conv.num + "_option1.html";
            option1list.push(option1name);
            var option2name = "/static/dataset_new/catch/conv" + conv.num + "_option2.html";
            option2list.push(option2name);
            var option3name = "/static/dataset_new/catch/conv" + conv.num + "_option3.html";
            option3list.push(option3name);
            var option4name = "/static/dataset_new/catch/conv" + conv.num + "_option4.html";
            option4list.push(option4name);
            var option5name = "/static/dataset_new/catch/conv" + conv.num + "_option5.html";
            option5list.push(option5name);
        } else if (conv.isFixed) {
            // For fixed conversations, use specific folders based on position
            var folderPath;
            if (i === 7) { // Position 7 is AIAI
                folderPath = "/static/dataset_new/conversation_24long_AIAI_formatted/";
            } else if (i === 15) { // Position 15 is Human-AI  
                folderPath = "/static/dataset_new/conversations_24long_human_AI_formatted/";
            } else if (i === 25) { // Position 25 is Human-Human
                folderPath = "/static/dataset_new/conversations_24long_human_human_formatted/";
            } else {
                // Fallback to regular path if position is unknown
                folderPath = "/static/dataset_new/";
            }
            
            var conversationname = folderPath + "conv" + conv.num + ".html";
            conversationlist.push(conversationname);
            
            var option1name = folderPath + "conv" + conv.num + "option1.html";
            option1list.push(option1name);
            var option2name = folderPath + "conv" + conv.num + "option2.html";
            option2list.push(option2name);
            var option3name = folderPath + "conv" + conv.num + "option3.html";
            option3list.push(option3name);
            var option4name = folderPath + "conv" + conv.num + "option4.html";
            option4list.push(option4name);
            var option5name = folderPath + "conv" + conv.num + "option5.html";
            option5list.push(option5name);
        } else {
            // For regular conversations, use length-specific files from dataset_new
            var conversationname = "/static/dataset_new/conv" + conv.num + "_len" + length + ".html";
            conversationlist.push(conversationname);
            
            var option1name = "/static/dataset_new/conv" + conv.num + "_len" + length + "option1.html";
            option1list.push(option1name);
            var option2name = "/static/dataset_new/conv" + conv.num + "_len" + length + "option2.html";
            option2list.push(option2name);
            var option3name = "/static/dataset_new/conv" + conv.num + "_len" + length + "option3.html";
            option3list.push(option3name);
            var option4name = "/static/dataset_new/conv" + conv.num + "_len" + length + "option4.html";
            option4list.push(option4name);
            var option5name = "/static/dataset_new/conv" + conv.num + "_len" + length + "option5.html";
            option5list.push(option5name);
        }
    }
...
var next = function() {
    ...
    function checkTopicCorrect(topic, conversationNum) {
        ...
        if ((conversationNum >= 65 && conversationNum <= 109)) {
          correctTopic = "1";
        } else if ((conversationNum >= 110 && conversationNum <= 173)) {
          correctTopic = "2";
        } else if ((conversationNum >= 174 && conversationNum <= 223)) {
          correctTopic = "3";
        } else if ((conversationNum >= 224 && conversationNum <= 273)) {
          correctTopic = "4";
        } else if ((conversationNum >= 274 && conversationNum <= 284) || 
                   (conversationNum >= 34 && conversationNum <= 63)) {
          correctTopic = "5";
        } else {
    ...
    function mengmiClick() {
        ...
        psiTurk.recordTrialData({
            ...
            'isFixed': selectedConvs[trialindex].isFixed || false,
            'fixedType': selectedConvs[trialindex].fixedType || null, 
