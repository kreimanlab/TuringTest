...
for imageid, entry in captions_val2017_anno_d.items():
    ...
    img_path = os.path.join(img_root,entry['file_name'])
    ...
    while (len(answer_text.split(' ')) > sampled_length or len(answer_text.split(' ')) < 6) and trial < 10:
        ...
        trial += 1
        print("[{}]".format(trial), imageid, sampled_length, len(answer_text.split(" ")), answer_text)
    answers[imageid] = answer_text
    # break
with open("/home/liuxiao/TuringGithub/XiaoData/all_results/llava-v1.6-mistral-7b_image_captioning_v2_cocoval2017_r3.8.pkl", 'wb') as f:
